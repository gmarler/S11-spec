...
if [[ "x$ZSH_VERSION" != x ]]; then
  ...
  for var in $ENV_SET; do
    case $var in
      HISTSIZE|HISTFILE|LANG|PS1|PS2|HZ|TERM|SHELL|OLDPWD|PATH|MAIL|PWD|TZ|SHLVL|HOME|LOGNAME|PRINTER|HOSTNAME|_|CC|CXX|EDITOR|SSH_*|DISPLAY|LESS*|LS_COLORS|LS_OPTIONS|TERMINFO|PAGER|MANPATH|VISUAL|XAUTHORITY|CVSROOT|CVS_RSH|DBUS_*|SESSION_MANAGER|JDS_CBE_ENV_QUIET|PKGBUILD_IPS_SERVER)
        ;;
      *)
