...
# Few bugs for Solaris:
# - isfinite() is only defined in c99, so must tell gcc to use the C99 standard
# - Must also define -D__EXTENSIONS__ to get that to work properly, since some
#   of the C99 extensions only activate with that flag (which the Studio
#   compiler would have already known to activate).
# - Need to add the -lnsl and -lsocket libs
#
MAKE=/bin/gmake \
CPP=gcpp \
./build.sh \
          ...
          --mandir=%{_mandir}/man1 \
          --cc=gcc \
          --cflags="-std=c99 -pipe -D_LARGEFILE_SOURCE -D_LARGEFILE64_SOURCE \
                    -D_FILE_OFFSET_BITS=64 -momit-leaf-frame-pointer \
                    -W -Wall -Wformat=2 -Wshadow -D__EXTENSIONS__" \
          --ldflags="-lnsl -lsocket" \
          --configure-only
#          --cppflags="-std=c99 -D__EXTENSIONS__" \
...
# Might need to remove in %install above - seems to just be symbols
%attr(0555, root, bin) %{_libdir}/%{short_name}/%{short_name}.nm
